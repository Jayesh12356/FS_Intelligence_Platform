...
# Shared serializer: ensure every outgoing datetime carries tzinfo. SQLite
# (used in tests and occasionally in dev) stores naive UTC; without this
# shim the API emits `"2026-04-18T10:12:01.123"` which violates RFC 3339
# date-time format declared in OpenAPI. Production Postgres stores tzaware
...
class FSDocumentResponse(BaseModel):
    ...
    created_at: datetime
    updated_at: datetime
    # True when the FS body changed (refine / accept-suggestion / …)
    # after the last successful analyze. The UI uses this to render a
    # "re-analyze to refresh metrics" banner while keeping the Build CTA
    # available — we no longer demote ``status`` on those edits.
...
class FSDocumentDetail(FSDocumentResponse):
    ...
    sections: List["FSSectionSchema"] | None = None
...
class GeneratedFSResponse(BaseModel):
    ...
    code_upload_id: UUID
    generated_fs_id: UUID | None = None
    status: str
    sections: List[FSSectionSchema] = []
    raw_text: str = ""
    report: CodeReportSchema | None = None
