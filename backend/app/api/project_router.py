...
async def create_project(
    ...
    existing = await db.execute(select(FSProject).where(FSProject.name == body.name))
    ...
    doc_count = (
        await db.execute(select(func.count(FSDocument.id)).where(FSDocument.project_id == project.id))
    ).scalar() or 0

    logger.info("Created project %s: %s", project.id, project.name)
    return APIResponse(
        data=FSProjectSchema(
            id=project.id,
            name=project.name,
...
async def list_projects(
    ...
    """List all projects."""
    result = await db.execute(select(FSProject).order_by(FSProject.updated_at.desc()))
...
async def get_project(
    ...
    result = await db.execute(select(FSProject).where(FSProject.id == project_id))
...
async def update_project(
    ...
    if body.name is not None:
        dup = await db.execute(select(FSProject).where(FSProject.name == body.name, FSProject.id != project_id))
...
async def delete_project(
    ...
    docs_result = await db.execute(select(FSDocument).where(FSDocument.project_id == project_id))
...
async def assign_document_to_project(
    ...
    proj_result = await db.execute(select(FSProject).where(FSProject.id == project_id))
    ...
    max_order = (
        await db.execute(select(func.max(FSDocument.order_in_project)).where(FSDocument.project_id == project_id))
    ).scalar() or 0
